...
% use loop to run one-vs-all
% k = class index; K = # of classes
for k = 1:num_labels
    y_normalized = y == k;
    cost1 = log(a3(k,:)) * y_normalized;
    cost2 = log(1 - a3(k,:)) * (1 - y_normalized);
    Jk = -1 / m * sum(cost1 + cost2);
    J = J + Jk;
end
...
for iter = 1:m
    ...
    y_vec(y(iter)) = 1;    
    delta3 = a_3 - y_vec; % 10 x 1
    
    delta2 = Theta2' * delta3; % 26 x 1
    delta2 = delta2(2:end) .* sigmoidGradient(z_2); % 25 x 1
